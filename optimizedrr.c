...
void calculate_rr_with_optimized_preemption(int n, struct process proc[], int tq) {
    int time = 0, count = 0;
    ...
    while (count < n) {
        ...
        for (int i = 0; i < n; i++) {
            if (proc[i].rt > 0 && proc[i].at <= time) {
                ...
                int time_exec = (proc[i].rt <= tq) ? proc[i].rt : tq;
                proc[i].rt -= time_exec;
                time += time_exec;
                // If process finishes
                if (proc[i].rt == 0) {
...
int main() {
    ...
    printf("\nExecuting processes using Optimized Round Robin:\n");
    calculate_rr_with_optimized_preemption(n, proc, tq);
    return 0;
}
